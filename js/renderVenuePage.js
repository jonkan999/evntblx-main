...
window.addEventListener("load", function () {
  ...
  function renderVenuePage() {
    ...
    for (let i = 2; i < Math.min(venueImages.images.length, 4); i++) {
      bottomRightImage = document.createElement("div");
    ...
    bottomRightImage.appendChild(showAllContainer);
    ...
    description.innerHTML = venueInfo.description.replace(/\\n/g, "<br>");

    descriptionContainer.appendChild(descriptionHeader);
    descriptionContainer.appendChild(description);

    /* Detailed information section */
    // Retrieve the stored venue information from local storage
    if (venueInfoJSON) {
      const venueInfo = JSON.parse(venueInfoJSON);

      // Create an HTML list to display the information
      const ul = document.createElement("ul");

      // Define properties to display as pairs of labels and property keys
      const propertiesToDisplay = [
        ...
        ["Mickar tillgängliga: ", "micsF"],
        ["Projektor tillgänglig: ", "projectorF"],
        ["Discoljus tillgängligt: ", "discoLightsF"],
